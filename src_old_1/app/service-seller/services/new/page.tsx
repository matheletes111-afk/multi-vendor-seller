...
async function createService(data: unknown) {
  ...
  try {
    await prisma.service.create({
      data: { sellerId: seller.id, categoryId: validated.data.categoryId, name: validated.data.name, slug, description: validated.data.description, serviceType: validated.data.serviceType, basePrice, discount, hasGst: validated.data.hasGst ?? true, duration: validated.data.duration, images: imagesData as any },
    })
...
async function createServiceForm(formData: FormData) {
  ...
  if (!name || !categoryId || !serviceType) redirect("/service-seller/services/new?error=missing_required_fields")
  ...
  const data = { name, description: (formData.get("description") as string) || undefined, categoryId, serviceType, basePrice, hasGst, discount, duration, images: images.length > 0 ? images : undefined }
