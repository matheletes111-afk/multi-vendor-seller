...
async function updateStore(data: { name?: string; description?: string; phone?: string; website?: string; address?: string; city?: string; state?: string; zipCode?: string; country?: string; logo?: string; banner?: string }) {
  ...
  try {
    if (seller.store) {
      await prisma.store.update({ where: { id: seller.store.id }, data: updateData })
    } else {
      await prisma.store.create({ data: { sellerId: seller.id, name: updateData.name || "My Store", ...updateData } })
    }
...
async function updateStoreForm(formData: FormData) {
  ...
  if (result.error) redirect(`/product-seller/settings?error=${encodeURIComponent(typeof result.error === "string" ? result.error : "Failed")}`)
  redirect("/product-seller/settings?success=Store information updated successfully")
}
...
async function updateUserForm(formData: FormData) {
  ...
  if (result.error) redirect(`/product-seller/settings?error=${encodeURIComponent(typeof result.error === "string" ? result.error : "Failed")}`)
  redirect("/product-seller/settings?success=Profile updated successfully")
}
