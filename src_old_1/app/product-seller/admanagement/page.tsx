...
async function getSellerAds() {
  ...
  if (!seller || seller.type !== "PRODUCT") return []
...
async function deleteAdForm(adId: string) {
  ...
  if (result.error) redirect(`/product-seller/admanagement?error=${encodeURIComponent(result.error)}`)
  redirect("/product-seller/admanagement?success=Ad deleted")
}
...
export default async function ProductSellerAdmanagementPage({
  ...
  return (
    <ProductSellerAdmanagementPageClient
      ads={ads}
      params={params}
      pauseAd={pauseAd}
      resumeAd={resumeAd}
