...
/**
 * Embedded Apple Root CA G2 & G3 certificates (DER format, base64 encoded)
 * Downloaded from official Apple PKI: https://www.apple.com/certificateauthority/
 * Embedded to guarantee offline JWS verification never fails in Serverless / Vercel / Container environments
 * even if the disk `certs/` folder is omitted from the deployment bundle.
 */
const FALLBACK_ROOT_CA_G2_BASE64 =
...
const FALLBACK_ROOT_CA_G3_BASE64 =
...
/**
 * Apple Root CA Certificates (Public Apple Root Certificates G2 & G3)
 * Used by @apple/app-store-server-library SignedDataVerifier to verify x5c certificate chains offline.
 */
export function getAppleRootCertificates(): Buffer[] {
  const certs: Buffer[] = []

  // Step 1: Attempt to load from disk if present
  try {
    const certDir = path.join(process.cwd(), "certs")
    const g2Path = path.join(certDir, "AppleRootCA-G2.cer")
    const g3Path = path.join(certDir, "AppleRootCA-G3.cer")

    if (fs.existsSync(g2Path)) {
      certs.push(fs.readFileSync(g2Path))
    }
    if (fs.existsSync(g3Path)) {
      certs.push(fs.readFileSync(g3Path))
    }
  } catch (err) {
    console.warn("[AppleIAP] Reading certs from disk encountered an error, falling back to embedded certs:", err)
  }

  // Step 2: Fallback to embedded certificates if disk files were not found
  if (certs.length === 0) {
    certs.push(Buffer.from(FALLBACK_ROOT_CA_G2_BASE64, "base64"))
    certs.push(Buffer.from(FALLBACK_ROOT_CA_G3_BASE64, "base64"))
  }
