...
export function CustomerAdmanagementPageClient() {
  ...
  const fetchAds = useCallback(() => {
    setLoading(true)
    return fetch(`/api/customer/admanagement?page=${page}&perPage=${perPage}`)
  ...
  async function pauseAd(adId: string) {
    const res = await fetch(`/api/customer/admanagement/${adId}`, {
      method: "PATCH",
  ...
  async function deleteAdForm(adId: string) {
    const res = await fetch(`/api/customer/admanagement/${adId}`, { method: "DELETE" })
    if (res.ok) {
      const wasLastOnPage = ads.length === 1
      if (wasLastOnPage && page > 1) {
        router.replace(`/customer/admanagement?page=${page - 1}`)
        return
      }
      fetchAds()
    }
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground mt-2">Own business ads with image or video (CPC)</p>
        </div>
        <Button asChild>
          <Link href="/customer/admanagement/new">
      ...
      {loading && ads.length === 0 ? (
        ...
        <Card>
          <CardContent className="py-16 text-center">
            ...
            <p className="text-muted-foreground mb-6">Create an ad with your image or video. You pay only when people click.</p>
            <Button asChild>
              <Link href="/customer/admanagement/new">
                <Plus className="mr-2 h-4 w-4" />
        ...
        <Card className="overflow-hidden">
          ...
          <div className="px-6 pb-6">
            <AdminPagination
              basePath="/customer/admanagement"
              currentPage={page}
