...
export default function CustomerRegistrationPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const res = await fetch("/api/customer/auth/registration", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name, email, phone: phone || undefined, phoneCountryCode: phoneCountryCode || undefined, password }) })
    ...
    <div className="flex min-h-screen min-w-0 items-start justify-center overflow-x-hidden bg-gray-50/90 px-4 py-6 sm:items-center sm:py-4">
      <div className="w-full max-w-[440px] min-w-0 max-h-[calc(100dvh-2rem)] overflow-y-auto rounded-2xl bg-white p-5 shadow-xl sm:rounded-3xl sm:p-6 md:p-8">
        ...
        <div className="mb-6 sm:mb-8">
          <h1 className="text-center text-xl font-semibold text-gray-900 sm:text-2xl">Create Account</h1>
          <p className="mt-1 text-center text-sm text-gray-500">Enter your information to get started</p>
        </div>
        <form onSubmit={handleSubmit}>
          ...
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-[1fr_2fr]">
              <div><Label htmlFor="phoneCountryCode" className="mb-1.5 block text-sm font-medium text-gray-700">Country code</Label><Input id="phoneCountryCode" type="tel" inputMode="numeric" placeholder="+1" value={phoneCountryCode} onChange={(e) => setPhoneCountryCode(e.target.value)} disabled={loading} className="rounded-xl border-gray-200" /></div>
              <div><Label htmlFor="phone" className="mb-1.5 block text-sm font-medium text-gray-700">Phone</Label><Input id="phone" type="tel" inputMode="numeric" placeholder="Optional" value={phone} onChange={(e) => setPhone(e.target.value)} disabled={loading} className="rounded-xl border-gray-200" /></div>
            </div>
          ...
          <p className="mt-6 text-center text-sm text-gray-600">Already have an account? <Link href="/customer/login" className="font-medium text-blue-600 hover:underline">Sign In</Link></p>
        </form>
      </div>
    </div>
  )
