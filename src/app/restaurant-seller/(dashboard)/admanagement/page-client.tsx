...
export function RestaurantSellerAdmanagementPageClient() {
  ...
  const fetchAds = useCallback(() => {
    setLoading(true)
    return fetch(`/api/restaurant-seller/admanagement?page=${page}&perPage=${perPage}`)
  ...
  async function pauseAd(adId: string) {
    const res = await fetch(`/api/restaurant-seller/admanagement/${adId}`, {
      method: "PATCH",
  ...
  async function deleteAdForm(adId: string) {
    const res = await fetch(`/api/restaurant-seller/admanagement/${adId}`, { method: "DELETE" })
    if (res.ok) {
      const wasLastOnPage = ads.length === 1
      if (wasLastOnPage && page > 1) {
        router.replace(`/restaurant-seller/admanagement?page=${page - 1}`)
        return
      }
      fetchAds()
    }
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground mt-2">Promote a food item or run your own image/video ad (CPC)</p>
        </div>
        <Button asChild className="rounded-xl shadow-lg">
          <Link href="/restaurant-seller/admanagement/new">
      ...
      {loading && ads.length === 0 ? (
        ...
        <Card className="rounded-3xl border-dashed">
          <CardContent className="py-16 text-center">
            ...
            <p className="text-muted-foreground mb-6">Create an ad for a food item or for your business. You pay only when customers click.</p>
        ...
        <Card className="overflow-hidden rounded-3xl border-none shadow-xl">
            ...
            <Table>
              <TableHeader className="bg-muted/40">
                <TableRow>
                  ...
                  <TableHead className="hidden md:table-cell">Food Item / own ad</TableHead>
                    ...
                    <TableRow key={ad.id} className="hover:bg-muted/10 transition-colors">
                      ...
                      <TableCell className="font-medium max-w-[200px]">
                        ...
                        <p className="text-xs text-muted-foreground md:hidden mt-0.5 line-clamp-1">
                          {ad.foodItem?.name ?? "Own business ad"}
                        </p>
                      </TableCell>
          ...
          <div className="px-6 pb-6">
            <AdminPagination
              basePath="/restaurant-seller/admanagement"
              currentPage={page}
