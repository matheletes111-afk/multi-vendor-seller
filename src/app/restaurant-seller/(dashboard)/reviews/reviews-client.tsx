...
import { Star, MessageSquare, Loader2, ArrowLeft, Calendar, ZoomIn } from "lucide-react"
...
type DetailResponse = {
  ...
  avgRating: string
  reviewCount: number
  reviews: FoodReviewDetail[]
}

export function RestaurantReviewsClient() {
  const [groupedItems, setGroupedItems] = useState<GroupedFoodItem[]>([])
  ...
  const fetchItemReviewDetails = async (itemId: string) => {
    setLoadingDetail(true)
    try {
      const res = await fetch(`/api/restaurant-seller/reviews/${itemId}`)
      const data = await res.json()
      if (data.success) {
  ...
  const averageRatingOverall = groupedItems.length > 0
    ? parseFloat((groupedItems.reduce((acc, item) => acc + parseFloat(item.avgRating), 0) / groupedItems.length).toFixed(1))
    : 0

  const totalReviewsCount = groupedItems.reduce((acc, item) => acc + item.reviewCount, 0)

  if (selectedItemId && detailData) {
    return (
        ...
        <div className="bg-white border border-slate-100 p-6 rounded-[2rem] shadow-sm flex flex-col md:flex-row items-center gap-6 justify-between">
          <div className="flex items-center gap-4">
            <div className="h-16 w-16 bg-slate-50 border border-slate-100 rounded-2xl overflow-hidden shrink-0 flex items-center justify-center text-xs font-bold text-slate-400">
              {detailData.foodItemImage ? (
                <img src={detailData.foodItemImage} alt={detailData.foodItemName} className="object-cover h-full w-full" />
              ) : (
                "Food"
              )}
            </div>
            <div>
              <h2 className="text-xl font-black text-slate-900 leading-tight">{detailData.foodItemName}</h2>
              <p className="text-slate-400 text-xs font-semibold mt-1">Average rating across {detailData.reviewCount} customer reviews</p>
            </div>
          </div>
                  ...
                  <div className="flex flex-wrap gap-2 mt-2.5">
                    {(review.images as string[]).map((img: string, idx: number) => (
                      <button
                        ...
                        className="group relative w-14 h-14 rounded-xl overflow-hidden border border-slate-100 shadow-xs shrink-0 bg-slate-50 hover:border-rose-500 transition-all cursor-pointer focus:outline-none ring-2 ring-transparent focus:ring-rose-500"
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-500 font-medium text-sm">Analyze feedback and star ratings from your customers item-wise.</p>
      </div>
      ...
      {loading ? (
        <div className="text-center py-20">
          ...
          <p className="text-slate-500 font-medium mt-3 text-sm">Loading reviews list...</p>
        </div>
        ...
        <Card className="rounded-[2rem] border-none shadow-sm text-center py-16">
          <CardContent className="space-y-4">
            ...
            <p className="text-slate-500 text-xs mt-1">Customers haven&apos;t left any ratings on your dishes yet.</p>
          </CardContent>
        </Card>
      ) : (
        <div className="grid gap-4 md:grid-cols-2">
          {groupedItems.map(item => (
            <Card key={item.foodItemId} className="rounded-3xl border-none shadow-sm bg-white p-5 flex flex-col justify-between gap-4 hover:shadow-md transition-shadow">
              <div className="flex items-center gap-3">
                <div className="h-12 w-12 bg-slate-50 border border-slate-100 rounded-xl overflow-hidden shrink-0 flex items-center justify-center text-[10px] font-bold text-slate-400">
                  {item.foodItemImage ? (
                    <img src={item.foodItemImage} alt={item.foodItemName} className="object-cover h-full w-full" />
                  ) : (
                    "Food"
                  )}
                </div>
                <div className="min-w-0 flex-1">
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">{item.foodItemCategory}</p>
                  <p className="text-sm font-black text-slate-800 truncate mt-0.5">{item.foodItemName}</p>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between border-t border-slate-50 pt-3">
                ...
                <Button
                  ...
                  onClick={() => fetchItemReviewDetails(item.foodItemId)}
