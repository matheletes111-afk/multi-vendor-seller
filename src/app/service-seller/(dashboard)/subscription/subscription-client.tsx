...
export function ServiceSubscriptionClient() {
    ...
    async function loadData() {
      try {
        const [subRes, plansRes] = await Promise.all([
          fetch("/api/service-seller/subscription"),
          fetch("/api/plans?type=PRODUCT_SERVICE"),
        ])
        const subData = await subRes.json()
        const plansData = await plansRes.json()

        setSubscription(subData && !subData.error ? subData : null)
  ...
  const handleSubscribe = async (planId: string) => {
    ...
    try {
      const res = await fetch("/api/service-seller/subscription/checkout", {
        method: "POST",
      ...
      if (data.url) {
        ...
        const subRes = await fetch("/api/service-seller/subscription")
        const subData = await subRes.json()
        setSubscription(subData && !subData.error ? subData : null)
      }
    } catch {
  ...
  return (
    <div className="container mx-auto p-8 space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-6">
        <div>
          <h1 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight">Subscription Panel</h1>
