...
export function ServiceSubscriptionClient() {
  ...
  useEffect(() => {
    Promise.all([
      fetch("/api/service-seller/subscription").then((r) => (r.ok ? r.json() : null)),
  ...
  return (
    <div className="container mx-auto p-8 space-y-8 animate-in fade-in duration-500">
      <div>
        ...
        <p className="text-slate-500 mt-2 font-medium">Select a plan to offer your services and handle appointments.</p>
      </div>
          ...
          <Card key={plan.id} className={`rounded-[2rem] border-2 shadow-xl overflow-hidden hover:shadow-2xl transition-all duration-300 ${subscription?.planId === plan.id ? "border-primary" : "border-slate-100"}`}>
            ...
            <CardContent>
              ...
              <ul className="space-y-3 mb-8 pt-4 border-t border-slate-50 font-medium text-slate-600">
                ...
                <li className="text-sm flex justify-between">
                  <span>Monthly Bookings/Orders:</span> 
