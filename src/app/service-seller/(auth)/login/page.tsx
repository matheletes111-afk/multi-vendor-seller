...
function ServiceSellerLoginForm() {
  ...
  const callbackUrl = getSafeRedirectUrl(searchParams.get("callbackUrl"), "/service-seller")
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const res = await fetch("/api/service-seller/auth/login", {
        method: "POST",
      ...
      if (res.status === 302) {
        ...
        if (loc && !loc.includes("error=")) {
          window.location.href = getSafeRedirectUrl(loc, "/service-seller")
          return
        }
      }
      if (res.ok) {
      ...
      <div className="w-full max-w-[440px] min-w-0 rounded-2xl bg-white p-5 shadow-xl sm:rounded-3xl sm:p-6 md:p-8">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="space-y-5">
            ...
            <div>
              ...
              <div className="mt-2 text-center">
                <Button type="button" variant="outline" asChild className="w-full rounded-full sm:max-w-[260px]">
                  <Link href={`/service-seller/login/phone-otp?callbackUrl=${encodeURIComponent(callbackUrl)}`}>Login via Number</Link>
                </Button>
              </div>
            </div>
          ...
          <p className="mt-6 text-center text-sm text-gray-600">
            ...
            <Link href="/service-seller/registration" className="font-medium text-blue-600 hover:underline">
              Register as Seller
            </Link>
          </p>
        </form>
...
export default function ServiceSellerLoginPage() {
  return (
