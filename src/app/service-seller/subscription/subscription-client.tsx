...
export function ServiceSubscriptionClient() {
  ...
  async function handleSubscribe(planName: string) {
    ...
    const res = await fetch("/api/service-seller/subscription/checkout", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ planName }) })
          ...
          <Card key={plan.id} className={subscription?.planId === plan.id ? "border-primary" : ""}>
            ...
            <CardContent>
              ...
              <ul className="space-y-2 mb-6">
                <li className="text-sm">Services: {plan.maxProducts === null ? "Unlimited" : plan.maxProducts}</li>
