...
/** POST /api/customer/auth/email-otp/verify-otp — Body: { email, otp } */
export async function POST(request: Request) {
  try {
    ...
    const otpLoginToken = createOtpLoginToken(email, UserRole.CUSTOMER)
    return NextResponse.json({ message: "OTP verified.", otpLoginToken, loginUrl: "/customer" }, { status: 200 })
  } catch (error) {
    console.error("Customer email-otp verify error:", error)
