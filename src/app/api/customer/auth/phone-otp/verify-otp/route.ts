...
/** POST /api/customer/auth/phone-otp/verify-otp — Body: { phone, otp } */
export async function POST(request: Request) {
  try {
    ...
    if (!user) return NextResponse.json({ error: "Invalid phone number or OTP." }, { status: 400 })
    ...
    console.error("Customer phone-otp verify error:", error)
