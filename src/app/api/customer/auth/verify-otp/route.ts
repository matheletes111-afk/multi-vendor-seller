...
/** POST /api/customer/auth/verify-otp — Body: { email, otp } */
export async function POST(request: Request) {
  try {
    ...
    if (user.isEmailVerified) return NextResponse.json({ message: "Already verified.", loginUrl: "/customer/login" }, { status: 200 })
    ...
    return NextResponse.json({ message: "Email verified.", loginUrl: "/customer/login?verified=1" }, { status: 200 })
  } catch (error) {
