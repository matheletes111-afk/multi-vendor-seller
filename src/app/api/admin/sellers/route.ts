  ...
  try {
    ...
    processedSellers.sort((a: any, b: any) => {
      switch (sortBy) {
        ...
        case "storename":
        case "businessname": {
          const valA = (a.businessInfo?.businessName || a.store?.name || a.user?.name || "").toLowerCase()
          const valB = (b.businessInfo?.businessName || b.store?.name || b.user?.name || "").toLowerCase()
          return valA.localeCompare(valB) * modifier
        }
        case "type":
        case "sellertype": {
