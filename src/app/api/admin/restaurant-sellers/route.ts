  ...
  try {
    ...
    if (search) {
      where = {
        ...
        OR: [
          ...
          { businessInfo: { landmark: { contains: search, mode: "insensitive" } } },
    ...
    processedSellers.sort((a: any, b: any) => {
      ...
      switch (sortBy) {
        ...
        case "restaurantname":
        case "businessname": {
          const valA = (a.businessInfo?.businessName || a.user?.name || "").trim().toLowerCase()
          const valB = (b.businessInfo?.businessName || b.user?.name || "").trim().toLowerCase()
          if (!valA && valB) return 1
          if (valA && !valB) return -1
          const diff = valA.localeCompare(valB) * modifier
          if (diff !== 0) return diff
