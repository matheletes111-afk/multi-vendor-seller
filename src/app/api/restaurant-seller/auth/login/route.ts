...
export async function POST(request: Request) {
  try {
    ...
    if (!rawIdentifier || (!password && !hasOtpLoginToken)) {
      return NextResponse.json({ error: "Email or mobile number, and password or OTP login token are required" }, { status: 400 })
    }
    ...
    if (user) {
      ...
      if (isCredentialsValid) {
        ...
        if (user.isEmailVerified === false) {
          ...
          const verifyUrl = `/restaurant-seller/verify-otp?${verifyParam}`
          return NextResponse.json(
