...
export async function POST(request: Request) {
  try {
    ...
    if (!email || (!password && !hasOtpLoginToken)) {
      return NextResponse.json({ error: "Email and password or OTP login token are required" }, { status: 400 })
    }
    ...
    if (user) {
      ...
      if (isCredentialsValid) {
        ...
        if (user.isEmailVerified === false) {
          const verifyUrl = `/restaurant-seller/verify-otp?email=${encodeURIComponent(cleanEmail)}`
          return NextResponse.json(
    ...
    console.error("Restaurant seller login error:", error)
