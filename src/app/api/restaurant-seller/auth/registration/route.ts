...
export async function POST(request: Request) {
  try {
    ...
    if (email && typeof email === "string" && email.trim()) {
      ...
      if (existingUser) return NextResponse.json({ error: "Email or mobile number is already registered" }, { status: 400 })
    ...
    if (normalizedEmail) {
      notificationPromises.push(
        sendVerificationOtpEmail({ to: normalizedEmail, otp: verifyEmailOtp, name: sanitizedName, verificationLink })
      )
    }
    ...
    return NextResponse.json({ 
      ...
      userId: user.id, 
      phone: normalizedPhone,
      verifyUrl: `/restaurant-seller/verify-otp?${verifyParam}` 
    }, { status: 201 })
  } catch (error) {
