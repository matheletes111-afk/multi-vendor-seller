...
/** POST /api/restaurant-seller/auth/verify-2fa — Verify 2FA OTP for Restaurant Seller web login */
export async function POST(request: Request) {
  try {
    ...
    return completeWebNextAuthLogin({
      ...
      callbackUrl,
      defaultPath: "/restaurant-seller",
      csrfToken,
    })
  } catch (error) {
    console.error("Restaurant seller 2FA verification error:", error)
    return NextResponse.json(
