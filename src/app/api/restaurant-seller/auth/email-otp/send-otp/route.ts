...
/** POST /api/restaurant-seller/auth/email-otp/send-otp — Body: { email } */
export async function POST(request: Request) {
  try {
    ...
    if (!user) {
      return NextResponse.json({ error: "No account found for this email in restaurant seller panel." }, { status: 404 })
    }
    ...
    console.error("Restaurant seller email-otp send error:", error)
