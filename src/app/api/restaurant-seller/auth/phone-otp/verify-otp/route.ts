...
export async function POST(request: Request) {
  try {
    ...
    const otpLoginToken = createOtpLoginToken(user.email, UserRole.SELLER_RESTAURANT)
    return NextResponse.json(
      { message: "OTP verified.", otpLoginToken, email: user.email, loginUrl: "/restaurant-seller" },
      { status: 200 }
    )
  } catch (error) {
