...
/** POST /api/restaurant-seller/auth/phone-otp/verify-otp — Body: { phone, otp } */
export async function POST(request: Request) {
  try {
    ...
    const otpLoginToken = createOtpLoginToken(user.email || user.phone, UserRole.SELLER_RESTAURANT, {
      userId: user.id,
      phone: user.phone,
      email: user.email,
    })
    return NextResponse.json(
      { message: "OTP verified.", otpLoginToken, email: user.email, phone: user.phone, loginUrl: "/restaurant-seller" },
      { status: 200 }
    )
  } catch (error) {
    console.error("Restaurant seller phone-otp verify error:", error)
