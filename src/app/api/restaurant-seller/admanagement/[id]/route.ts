...
export async function PATCH(
  ...
  try {
    ...
    if (foodItemId) {
      ...
      if (!food) return NextResponse.json({ error: "Food item not found or does not belong to you" }, { status: 404 })
    }
    ...
    console.error("Restaurant seller edit ad error:", err)
...
export async function DELETE(
  ...
  const { id } = await params
  const seller = await prisma.restaurantSeller.findUnique({ where: { userId: session.user.id } })
  if (!seller) return NextResponse.json({ error: "Seller not found" }, { status: 404 })
  const ad = await prisma.sellerAd.findFirst({ where: { id, restaurantSellerId: seller.id } })
