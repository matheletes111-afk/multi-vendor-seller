...
/** POST /api/service-seller/auth/phone-otp/verify-otp — Body: { phone, otp } */
export async function POST(request: Request) {
  try {
    ...
    const otpLoginToken = createOtpLoginToken(user.email || user.phone, UserRole.SELLER_SERVICE, {
      userId: user.id,
      phone: user.phone,
      email: user.email,
    })
    return NextResponse.json(
      { message: "OTP verified.", otpLoginToken, email: user.email, phone: user.phone, loginUrl: "/service-seller" },
      { status: 200 }
    )
  } catch (error) {
    console.error("Service seller phone-otp verify error:", error)
