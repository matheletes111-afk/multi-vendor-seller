...
/** POST /api/service-seller/auth/phone-otp/send-otp — Body: { phone } */
export async function POST(request: Request) {
  try {
    ...
    if (!user) {
      return NextResponse.json(
        { error: "No service seller account found with this phone number." },
        { status: 404 }
      )
    }
    ...
    console.error("Service seller phone-otp send error:", error)
