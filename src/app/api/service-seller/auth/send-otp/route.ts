...
/** POST /api/service-seller/auth/send-otp — Body: { email } */
export async function POST(request: Request) {
  try {
    ...
    if (user.isEmailVerified) return NextResponse.json({ message: "Email is already verified.", loginUrl: "/service-seller/login" }, { status: 200 })
    const now = new Date()
