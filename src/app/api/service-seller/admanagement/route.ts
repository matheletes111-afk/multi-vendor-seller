...
export async function POST(request: NextRequest) {
  ...
  if (contentType.includes("multipart/form-data")) {
    ...
    body = {
      adType: (formData.get("adType") as string) || "promote_service",
  ...
  const adTypeRaw = String(body.adType ?? "promote_service").trim().toLowerCase().replace(/-/g, "_")
  const isOwnAd = adTypeRaw === "own_ad" || adTypeRaw === "ownad"
  const serviceIdRaw = String(body.serviceId ?? "").trim()
  ...
  if (!isOwnAd && !serviceIdRaw) {
    return NextResponse.json({ error: "serviceId is required when promoting a service" }, { status: 400 })
  }
  if (!title) {
  ...
  const resolvedServiceId = isOwnAd ? null : serviceIdRaw
  if (!isOwnAd && resolvedServiceId) {
    const service = await prisma.service.findFirst({
      where: { id: resolvedServiceId, sellerId: seller.id },
    })
