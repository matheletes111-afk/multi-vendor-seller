...
export async function POST(request: NextRequest) {
  ...
  else if (section === "store" || section === "categories") step = 5
  ...
  if (hasAny("paymentOption", "mobileMoneyOption", "bankName", "accountHolderName", "accountNumber", "bbanNumber", "branchName", "bankAddress", "bankPassbook", "passbook", "bankLetter", "preferredPayoutMethod") ||
    ...
    step = 6
  } else if (hasAny("storeLogo", "storeBanner", "categoryIds", "customCategories", "suggestionCount") || (hasField("storeName") && !hasField("businessRegNumber"))) {
    step = 5
  ...
  try {
    if (step === 2) {
      ...
      await (prisma as any).sellerBusinessInfo.upsert({
        ...
        update: { ...data, busRegCertUrl, cityCouncilCertUrl, gstTinCertUrl, addressProofUrl } as any,
        create: { ...data, busRegCertUrl, cityCouncilCertUrl, gstTinCertUrl, addressProofUrl, sellerId: seller.id } as any,
      })
    } else if (step === 3) {
      // Step 3: KYC
