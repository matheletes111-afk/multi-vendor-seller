...
/** POST /api/product-seller/auth/send-otp — Body: { email, phone, phoneCountryCode } */
export async function POST(request: Request) {
  try {
    ...
    if (user.isEmailVerified) return NextResponse.json({ message: "Account is already verified.", loginUrl: "/product-seller/login" }, { status: 200 })
    const now = new Date()
    ...
    console.error("Product seller send-otp error:", error)
