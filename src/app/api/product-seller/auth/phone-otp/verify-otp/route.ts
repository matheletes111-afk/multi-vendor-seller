...
/** POST /api/product-seller/auth/phone-otp/verify-otp — Body: { phone, otp } */
export async function POST(request: Request) {
  try {
    ...
    const otpLoginToken = createOtpLoginToken(user.email || user.phone, UserRole.SELLER_PRODUCT, {
      userId: user.id,
      phone: user.phone,
      email: user.email,
    })
    return NextResponse.json(
      { message: "OTP verified.", otpLoginToken, email: user.email, phone: user.phone, loginUrl: "/product-seller" },
      { status: 200 }
    )
  } catch (error) {
    console.error("Product seller phone-otp verify error:", error)
