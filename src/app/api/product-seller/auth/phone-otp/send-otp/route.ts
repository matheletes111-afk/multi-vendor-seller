...
/** POST /api/product-seller/auth/phone-otp/send-otp — Body: { phone } */
export async function POST(request: Request) {
  try {
    ...
    if (!user) {
      return NextResponse.json(
        { error: "No product seller account found with this phone number." },
        { status: 404 }
      )
    }
    ...
    console.error("Product seller phone-otp send error:", error)
