...
/** POST /api/product-seller/auth/verify-2fa — Verify 2FA OTP for Product Seller web login */
export async function POST(request: Request) {
  try {
    ...
    return completeWebNextAuthLogin({
      ...
      callbackUrl,
      defaultPath: "/product-seller",
      csrfToken,
    })
  } catch (error) {
    console.error("Product seller 2FA verification error:", error)
    return NextResponse.json(
