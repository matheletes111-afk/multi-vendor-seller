...
/** POST /api/product-seller/auth/email-otp/verify-otp — Body: { email, otp } */
export async function POST(request: Request) {
  try {
    ...
    const otpLoginToken = createOtpLoginToken(email, UserRole.SELLER_PRODUCT)
    return NextResponse.json({ message: "OTP verified.", otpLoginToken, loginUrl: "/product-seller" }, { status: 200 })
  } catch (error) {
    console.error("Product seller email-otp verify error:", error)
