...
/** POST /api/hotel-seller/auth/send-otp — Body: { email, phone, phoneCountryCode } */
export async function POST(request: Request) {
  try {
    ...
    if (user.isEmailVerified) return NextResponse.json({ message: "Account is already verified.", loginUrl: "/hotel-seller/login" }, { status: 200 })
    ...
    console.error("Hotel seller send-otp error:", error)
