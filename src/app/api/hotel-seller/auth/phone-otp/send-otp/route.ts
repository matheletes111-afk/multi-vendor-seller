...
/** POST /api/hotel-seller/auth/phone-otp/send-otp — Body: { phone } */
export async function POST(request: Request) {
  try {
    ...
    if (!user) {
      return NextResponse.json(
        { error: "No hotel seller account found with this phone number." },
        { status: 404 }
      )
    }
    ...
    await sendSmsViaTwilio({
      to: normalizedPhone,
      body: `Your hotel seller login OTP is ${otp}. It expires in 10 minutes.`,
    })

    return NextResponse.json({ message: "OTP sent to your phone." }, { status: 200 })
  } catch (error) {
    console.error("Hotel seller phone-otp send error:", error)
