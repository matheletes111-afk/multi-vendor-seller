...
/** POST /api/hotel-seller/auth/email-otp/send-otp — Body: { email } */
export async function POST(request: Request) {
  try {
    ...
    if (!user) {
      return NextResponse.json({ error: "No account found for this email in hotel seller panel." }, { status: 404 })
    }
    ...
    console.error("Hotel seller email-otp send error:", error)
