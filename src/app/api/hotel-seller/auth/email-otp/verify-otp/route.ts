...
/** POST /api/hotel-seller/auth/email-otp/verify-otp — Body: { email, otp } */
export async function POST(request: Request) {
  try {
    ...
    const otpLoginToken = createOtpLoginToken(email, UserRole.SELLER_HOTEL)
    return NextResponse.json({ message: "OTP verified.", otpLoginToken, loginUrl: "/hotel-seller" }, { status: 200 })
  } catch (error) {
    console.error("Hotel seller email-otp verify error:", error)
