...
export async function PATCH(
  ...
  try {
    ...
    if (hotelId) {
      ...
      if (!hotel) return NextResponse.json({ error: "Hotel property not found or does not belong to you" }, { status: 404 })
    }
    ...
    console.error("Hotel seller edit ad error:", err)
...
export async function DELETE(
  ...
  const { id } = await params
  const seller = await prisma.hotelSeller.findUnique({ where: { userId: session.user.id } })
  if (!seller) return NextResponse.json({ error: "Seller not found" }, { status: 404 })
  const ad = await prisma.sellerAd.findFirst({ where: { id, hotelSellerId: seller.id } })
