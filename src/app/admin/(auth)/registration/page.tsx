...
export default function AdminRegistrationPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!/^\+?[0-9]+$/.test(phoneCountryCode.trim())) { setError("Country code must contain only numbers (optionally starting with +)"); return }
    if (!/^[0-9]+$/.test(phone.trim())) { setError("Phone number must contain only numbers"); return }
    if (password !== confirmPassword) { setError("Passwords do not match"); return }
    if (password.length < 6) { setError("Password must be at least 6 characters"); return }
    setLoading(true)
    try {
      const res = await fetch("/api/admin/auth/registration", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name, email, phone: phone.trim(), phoneCountryCode: phoneCountryCode.trim(), password }) })
      const data = await res.json()
      if (!res.ok) { setError(data.error || "Registration failed"); return }
      router.push(`/admin/verify-otp?email=${encodeURIComponent(email)}&from=registration`)
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-[1fr_2fr]">
              ...
              <div><Label htmlFor="phone" className="mb-1.5 block text-sm font-medium text-gray-700">Phone</Label><Input id="phone" type="tel" inputMode="numeric" placeholder="Phone number" value={phone} onChange={(e) => setPhone(e.target.value.replace(/[^\d]/g, ""))} pattern="^[0-9]+$" title="Phone number must contain only numbers." required disabled={loading} className="rounded-xl border-gray-200" /></div>
            </div>
