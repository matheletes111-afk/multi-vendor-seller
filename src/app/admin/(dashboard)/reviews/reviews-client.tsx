...
export function AdminReviewsClient() {
  ...
  useEffect(() => {
    let cancelled = false
    setLoading(true)
    fetch(`/api/admin/reviews?page=${page}&perPage=${perPage}`, { credentials: "include" })
  ...
  const shouldShowSeeMore = (row: ReviewGroupRow) => (row.itemName ?? "").length > 26
                    ...
                    <TableCell className="font-medium max-w-[360px]">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col gap-1 min-w-0">
                          <span className="break-words">{itemName(row)}</span>
