...
function ProductSellerLoginForm() {
  ...
  useEffect(() => {
    ...
    } else if (err === "EmailAlreadyUsedAsServiceSeller") {
      setError("This email is already registered as a Service Seller. Please use a different email to sign in as a Product Seller.")
    } else if (err === "EmailAlreadyUsedAsCustomer") {
      setError("This email is already registered as a Customer. Please sign in using the Customer login page or use a different email for Product Seller.")
    }
  }, [searchParams])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const res = await fetch("/api/product-seller/auth/login", {
        method: "POST",
          ...
          <div className="space-y-5">
            ...
            <div>
              ...
              <div className="mt-2 text-right">
                <Link href="/product-seller/forgot-password" className="text-sm font-medium text-blue-600 hover:underline">
                  Forgot password?
                </Link>
              </div>
              ...
              <div className="mt-3 text-center">
                <Button type="button" variant="outline" asChild className="w-full rounded-full sm:max-w-[260px]">
                  <Link href={`/product-seller/login/email-otp?callbackUrl=${encodeURIComponent(callbackUrl)}`}>Login via Email OTP</Link>
                </Button>
              </div>
              <div className="mt-2 text-center">
                <Button type="button" variant="outline" asChild className="w-full rounded-full sm:max-w-[260px]">
                  <Link href={`/product-seller/login/phone-otp?callbackUrl=${encodeURIComponent(callbackUrl)}`}>Login via Number</Link>
                </Button>
              </div>
            </div>
              ...
              <Button
                ...
                onClick={async () => {
                  try {
                    ...
                    const callbackUrlWithRole = `/api/auth/oauth-postprocess?role=SELLER_PRODUCT&next=${nextUrl}`
