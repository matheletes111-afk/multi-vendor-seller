...
/**
 * GET /mobileapi/restaurant-seller/admanagement
 * List ads for the authenticated restaurant seller.
 */
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const where = { restaurantSellerId: seller.id }
...
/**
 * POST /mobileapi/restaurant-seller/admanagement
  ...
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      body = {
        ...
        foodItemId: (formData.get("foodItemId") as string) || (formData.get("foodId") as string) || (formData.get("productId") as string),
    ...
    if (!isOwnAd && !foodItemIdRaw) {
      return NextResponse.json({ success: false, error: "foodItemId is required when promoting a food item" }, { status: 400 })
    }
    if (!title) {
    ...
    if (!isOwnAd && resolvedFoodItemId) {
      const foodItem = await prisma.foodItem.findFirst({
        where: { id: resolvedFoodItemId, restaurantSellerId: seller.id, isDeleted: false },
      })
      if (!foodItem) {
