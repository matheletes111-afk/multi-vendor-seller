...
/** POST /mobileapi/restaurant-seller/auth/phone-otp/verify-otp — Body: { phone, otp } */
export async function POST(request: Request): Promise<NextResponse<ApiResponse>> {
  try {
    ...
    if (!user.restaurantSeller) {
      return NextResponse.json({ success: false, error: "Restaurant seller account not properly configured. Please contact support." }, { status: 403 })
    }
    ...
    console.error("Mobile restaurant seller phone-otp verify error:", error)
