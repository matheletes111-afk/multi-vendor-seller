...
interface SuccessResponse {
  ...
  data: {
    user: {
      ...
      createdAt: Date
      updatedAt: Date
      sellerInfo: RestaurantSellerInfo | null
    }
  ...
  try {
      ...
      include: {
        user: {
          select: {
            ...
            createdAt: true,
            updatedAt: true,
            restaurantSeller: { select: { isApproved: true, isSuspended: true, onboardingCompleted: true, onboardingStep: true } },
          },
        },
      },
    })
    ...
    if (existingAccount) {
      ...
      if (u.role !== UserRole.SELLER_RESTAURANT) {
        return NextResponse.json<ErrorResponse>(
          { success: false, error: "This account is not a restaurant seller account" },
          { status: 403 }
        )
      }
      if (!u.restaurantSeller) {
        return NextResponse.json<ErrorResponse>(
          { success: false, error: "Restaurant seller account not properly configured. Please contact support." },
          { status: 403 }
        )
      }
      if (!u.restaurantSeller.isApproved) {
        return NextResponse.json<ErrorResponse>(
          {
            success: false,
      ...
      if (u.restaurantSeller.isSuspended) {
        return NextResponse.json<ErrorResponse>(
          {
            success: false,
      ...
      return NextResponse.json<SuccessResponse>(
        {
          ...
          data: {
            user: {
              ...
              sellerInfo: {
                ...u.restaurantSeller,
                mobileStep: Math.max(1, (u.restaurantSeller?.onboardingStep || 1) - 1)
              },
            },
            tokens,
    ...
    const existingUser = await prisma.user.findUnique({
      where: { email },
      include: { restaurantSeller: true },
    })
    if (existingUser) {
      if (existingUser.role !== UserRole.SELLER_RESTAURANT) {
        return NextResponse.json<ErrorResponse>(
      ...
      const restaurantSeller = existingUser.restaurantSeller
      if (!restaurantSeller) {
        return NextResponse.json<ErrorResponse>(
          { success: false, error: "Restaurant seller account not properly configured. Please contact support." },
          { status: 403 }
      ...
      if (!restaurantSeller.isApproved) {
        return NextResponse.json<ErrorResponse>(
          {
            success: false,
            error: "Your account is pending admin approval. You cannot log in until approved.",
      ...
      if (restaurantSeller.isSuspended) {
        return NextResponse.json<ErrorResponse>(
          {
            success: false,
            error: "Your account has been suspended. Please contact support.",
      ...
      return NextResponse.json<SuccessResponse>(
        {
          ...
          data: {
            user: {
              ...
              sellerInfo: {
                ...restaurantSeller,
                mobileStep: Math.max(1, (restaurantSeller?.onboardingStep || 1) - 1)
              },
            },
            tokens,
            sessionInfo: { expiresIn: tokens.expiresIn, tokenType: "Bearer" },
    ...
    console.error("Mobile restaurant seller social login error:", error)
    return NextResponse.json<ErrorResponse>(
