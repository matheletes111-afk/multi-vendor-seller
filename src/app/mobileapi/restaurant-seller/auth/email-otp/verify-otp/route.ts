...
/** POST /mobileapi/restaurant-seller/auth/email-otp/verify-otp — Body: { email, otp } */
export async function POST(request: Request): Promise<NextResponse<ApiResponse>> {
  try {
    ...
    if (!user) return NextResponse.json({ success: false, error: "No account found for this email in restaurant seller panel." }, { status: 404 })
    ...
    if (!user.restaurantSeller) return NextResponse.json({ success: false, error: "Restaurant seller account not properly configured. Please contact support." }, { status: 403 })
    ...
    return NextResponse.json({
      ...
      data: {
        user: {
          ...
          sellerInfo: {
            ...user.restaurantSeller,
            mobileStep: Math.max(1, (user.restaurantSeller?.onboardingStep || 1) - 1)
          },
        },
        tokens,
    ...
    console.error("Mobile restaurant seller email-otp verify error:", error)
