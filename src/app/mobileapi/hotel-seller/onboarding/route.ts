...
export async function GET(request: NextRequest) {
    ...
    const sectionNames = ["business", "kyc", "property", "bank", "agreement"];
...
export async function POST(request: NextRequest) {
    ...
    else if (section === "property" || section === "hotel" || section === "categories") step = 4;
    ...
    if (!step && (
        ...
        (hasField("mobileNumber") && !hasField("businessName") && !hasField("managerName")) ||
    ...
    // 3. Signature detection for Property Setup (Step 4)
    if (!step && (hasAny("estimateHotelCount", "estimateRoomCount", "mainPhoto", "logo", "banner") || (hasField("categories") && !hasField("businessRegNumber")))) {
        step = 4;
    }
    ...
    if (!step && hasAny("businessName", "businessType", "businessRegNumber", "busRegCert", "taxIdNumber", "landmark", "managerName", "pocContact", "haveGst", "cityCouncilCert", "addressProof")) {
        step = 2;
    }

    // Fallback 1: Explicit 1-based parameter (mobileStep or currentStep where 1=Business, 2=KYC, 3=Property, 4=Bank, 5=Agreement)
    ...
    try {
        if (step === 2) {
            ...
            if (!logoUrl || !mainPhotoUrl) {
                return NextResponse.json({ success: false, error: "Property Logo and Main Property Photo are mandatory." }, { status: 400 });
            }

            if (!categories || categories.length === 0) {
                return NextResponse.json({ success: false, error: "Please select at least one property category." }, { status: 400 });
            }
