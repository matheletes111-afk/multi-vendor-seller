...
/** POST /mobileapi/hotel-seller/auth/forgot-password/reset */
export async function POST(request: Request): Promise<NextResponse<ApiResponse>> {
  try {
    ...
    return NextResponse.json(
      { success: true, message: "Password reset successful.", data: { loginUrl: "/hotel-seller/login?reset=1" } },
      { status: 200 }
    )
  } catch (error) {
    console.error("Mobile hotel-seller forgot-password reset error:", error)
