...
/** POST /mobileapi/hotel-seller/auth/email-otp/verify-otp — Body: { email, otp } */
export async function POST(request: Request): Promise<NextResponse<ApiResponse>> {
  try {
    ...
    if (!user) return NextResponse.json({ success: false, error: "No account found for this email in hotel seller panel." }, { status: 404 })
    ...
    if (!user.hotelSeller) return NextResponse.json({ success: false, error: "Hotel seller account not properly configured. Please contact support." }, { status: 403 })
    ...
    console.error("Mobile hotel seller email-otp verify error:", error)
