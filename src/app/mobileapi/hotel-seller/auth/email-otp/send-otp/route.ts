...
/** POST /mobileapi/hotel-seller/auth/email-otp/send-otp — Body: { email } */
export async function POST(request: Request): Promise<NextResponse<ApiResponse>> {
  try {
    ...
    if (!user) {
      return NextResponse.json({ success: false, error: "No account found for this email in hotel seller panel." }, { status: 404 })
    }
    ...
    console.error("Mobile hotel seller email-otp send error:", error)
