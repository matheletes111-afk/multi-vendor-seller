...
interface SuccessResponse {
  ...
  data: {
    user: {
      ...
      createdAt: Date
      updatedAt: Date
      sellerInfo: HotelSellerInfo | null
    }
  ...
  try {
    ...
    if (existingAccount) {
      ...
      if (u.role !== UserRole.SELLER_HOTEL) {
        return NextResponse.json<ErrorResponse>(
          { success: false, error: "This account is not a hotel seller account" },
          { status: 403 }
        )
      }
      if (!u.hotelSeller) {
        return NextResponse.json<ErrorResponse>(
          { success: false, error: "Hotel seller account not properly configured. Please contact support." },
          { status: 403 }
        )
      }
      if (!u.hotelSeller.isApproved) {
        return NextResponse.json<ErrorResponse>(
          {
            success: false,
      ...
      return NextResponse.json<SuccessResponse>(
        {
          ...
          data: {
            user: {
              ...
              sellerInfo: {
                ...u.hotelSeller,
                mobileStep: Math.max(1, (u.hotelSeller?.onboardingStep || 1) - 1)
              },
            },
            tokens,
    ...
    const existingUser = await prisma.user.findUnique({
      where: { email },
      include: { hotelSeller: true },
    })
    if (existingUser) {
      if (existingUser.role !== UserRole.SELLER_HOTEL) {
        return NextResponse.json<ErrorResponse>(
      ...
      const hotelSeller = existingUser.hotelSeller
      if (!hotelSeller) {
        return NextResponse.json<ErrorResponse>(
          { success: false, error: "Hotel seller account not properly configured. Please contact support." },
          { status: 403 }
      ...
      return NextResponse.json<SuccessResponse>(
        {
          ...
          data: {
            user: {
              ...
              sellerInfo: {
                ...hotelSeller,
                mobileStep: Math.max(1, (hotelSeller?.onboardingStep || 1) - 1)
              },
            },
            tokens,
            sessionInfo: { expiresIn: tokens.expiresIn, tokenType: "Bearer" },
    ...
    console.error("Mobile hotel seller social login error:", error)
    return NextResponse.json<ErrorResponse>(
