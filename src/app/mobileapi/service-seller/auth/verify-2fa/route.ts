...
/** POST /mobileapi/service-seller/auth/verify-2fa — Verify 2FA OTP for Service Seller mobile login */
export async function POST(request: Request) {
  try {
    ...
    const { password: _, seller, ...userWithoutPassword } = user as any
        ...
        data: {
          user: {
            ...
            sellerInfo: seller
              ? {
                  ...seller,
                  mobileStep: Math.max(1, (seller.onboardingStep || 2) - 1),
                }
              : null,
          },
          tokens,
    ...
    console.error("Mobile service-seller 2FA verification error:", error)
    return NextResponse.json(
