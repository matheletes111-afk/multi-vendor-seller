  ...
  try {
    ...
    if (existingAccount) {
      ...
      if (u.role !== UserRole.SELLER_SERVICE) {
        return NextResponse.json<ErrorResponse>(
          { success: false, error: "This account is not a service seller account" },
          { status: 403 }
        )
      }
      if (!u.seller) {
    ...
    if (existingUser) {
      if (existingUser.role !== UserRole.SELLER_SERVICE) {
        return NextResponse.json<ErrorResponse>(
    ...
    const newSeller = await prisma.seller.create({
      data: { userId: newUser.id, type: "SERVICE" },
      select: { id: true, isApproved: true, isSuspended: true, onboardingCompleted: true, onboardingStep: true, type: true },
    })
    await activateFreePlan(newSeller.id)
    await prisma.account.create({
      data: {
        userId: newUser.id,
        type: "oauth",
    ...
    console.error("Mobile service seller social login error:", error)
    return NextResponse.json<ErrorResponse>(
