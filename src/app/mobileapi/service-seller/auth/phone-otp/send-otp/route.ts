...
/** POST /mobileapi/service-seller/auth/phone-otp/send-otp — Body: { phone }. Same rules as web /api/service-seller/auth/phone-otp/send-otp */
export async function POST(request: Request): Promise<NextResponse<ApiResponse>> {
  try {
    ...
    if (!user) {
      return NextResponse.json(
        { success: false, error: "No service seller account found with this phone number." },
        { status: 404 }
      )
    }
    ...
    console.error("Mobile service seller phone-otp send error:", error)
