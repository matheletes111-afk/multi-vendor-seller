...
export async function GET(request: NextRequest) {
    ...
    const sectionNames = ["business", "kyc", "bank", "service", "agreement"];
...
export async function POST(request: NextRequest) {
    ...
    else if (section === "service" || section === "store" || section === "categories") mobileStep = 4;
    ...
    // 3. Signature detection for Service Setup & Categories (mobileStep = 4, dbStep = 5)
    if (!mobileStep && (hasAny("storeLogo", "storeBanner", "selectedServiceCategories", "categoryIds", "customCategories", "suggestionCount") || (hasField("storeName") && !hasField("businessRegNumber")))) {
        mobileStep = 4;
    }
