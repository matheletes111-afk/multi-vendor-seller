...
/**
 * GET /mobileapi/service-seller/settings
...
/**
 * PUT /mobileapi/service-seller/settings
 * Update settings for the authenticated service seller.
  ...
  try {
    ...
    if (sData) {
      ...
      if (sData.categoryIds && Array.isArray(sData.categoryIds)) {
        finalSellerUpdate.selectedServiceCategories = { set: sData.categoryIds.map((id: string) => ({ id })) }
      }
    }
