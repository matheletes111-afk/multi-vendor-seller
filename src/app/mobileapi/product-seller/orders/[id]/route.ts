...
export async function GET(
  ...
  if (staleOffers.length > 0) {
    ...
    for (const offer of staleOffers) {
      ...
      if (!isTerminalOrder && !hasAcceptedRider) {
        ...
        console.log(
          `[Mobile GET] Stale offer ${offer.id} expired on order ${orderId} (Terminal: ${isTerminalOrder}, Accepted: ${!!hasAcceptedRider}). No re-cascade.`
        )
      }
    }
  }
  ...
  if (activeAssignment) {
    ...
    activeDeliveryTracking = {
      ...
      pickupProofPhotos: activeAssignment.pickupProofPhotos || [],
      rider: {
        id: r?.id,
