...
/** POST /mobileapi/product-seller/auth/forgot-password/reset */
export async function POST(request: Request): Promise<NextResponse<ApiResponse>> {
  try {
    ...
    return NextResponse.json(
      { success: true, message: "Password reset successful.", data: { loginUrl: "/product-seller/login?reset=1" } },
      { status: 200 }
    )
  } catch (error) {
    console.error("Mobile product-seller forgot-password reset error:", error)
