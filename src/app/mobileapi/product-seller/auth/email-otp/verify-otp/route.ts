...
/** POST /mobileapi/product-seller/auth/email-otp/verify-otp — Body: { email, otp } */
export async function POST(request: Request): Promise<NextResponse<ApiResponse>> {
  try {
    ...
    if (!user) return NextResponse.json({ success: false, error: "No account found for this email in product seller panel." }, { status: 404 })
    ...
    console.error("Mobile product seller email-otp verify error:", error)
