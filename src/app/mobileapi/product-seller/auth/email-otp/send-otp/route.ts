...
/** POST /mobileapi/product-seller/auth/email-otp/send-otp — Body: { email } */
export async function POST(request: Request): Promise<NextResponse<ApiResponse>> {
  try {
    ...
    if (!user) {
      return NextResponse.json({ success: false, error: "No account found for this email in product seller panel." }, { status: 404 })
    }
    if (!user.isEmailVerified) {
      return NextResponse.json(
        { 
          ...
          verifyUrl: "/mobileapi/product-seller/auth/verify-otp",
          data: { email }
        }, 
        { status: 403 }
      )
    ...
    console.error("Mobile product seller email-otp send error:", error)
