...
/**
 * GET /mobileapi/product-seller/ads
 * List ads for the authenticated product seller.
 */
...
/**
 * POST /mobileapi/product-seller/ads
 * Create a new ad (promote product or own business).
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!isOwnAd && !productId) return NextResponse.json({ success: false, error: "Product ID is required for promoting a product" }, { status: 400 })
    ...
    console.error("Mobile product ad creation error:", error)
