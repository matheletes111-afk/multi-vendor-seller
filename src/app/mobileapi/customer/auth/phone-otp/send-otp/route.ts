...
/** POST /mobileapi/customer/auth/phone-otp/send-otp — Body: { phone } (E.164). Same rules as web /api/customer/auth/phone-otp/send-otp */
export async function POST(request: Request): Promise<NextResponse<ApiResponse>> {
  try {
    ...
    const user = await prisma.user.findFirst({
      ...
      select: { id: true, emailOtpSentAt: true },
    })

    if (!user) {
      return NextResponse.json(
        { success: false, error: "No customer account found with this phone number." },
        { status: 404 }
      )
    }
    ...
    console.error("Mobile customer phone-otp send error:", error)
