...
/** POST /mobileapi/customer/auth/email-otp/send-otp — Body: { email } */
export async function POST(request: Request): Promise<NextResponse<ApiResponse>> {
  try {
    ...
    if (!user) {
      return NextResponse.json({ success: false, error: "No account found for this email in customer panel." }, { status: 404 })
    }
    ...
    console.error("Mobile customer email-otp send error:", error)
