...
function HotelSellerResetPasswordForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const res = await fetch("/api/hotel-seller/auth/forgot-password/reset", {
        method: "POST",
      ...
      router.push(data.loginUrl || "/hotel-seller/login?reset=1")
    } catch {
  ...
  const resendOtp = async () => {
    ...
    setLoading(true)
    try {
      const res = await fetch("/api/hotel-seller/auth/forgot-password/send-otp", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  if (!identifier) return <div className="flex min-h-screen items-center justify-center bg-gray-50/90 p-4"><div className="w-full max-w-[440px] rounded-3xl bg-white p-8 shadow-xl text-center"><p className="text-gray-600">Missing email or mobile number. Please start from forgot password.</p><Link href="/hotel-seller/forgot-password" className="mt-4 inline-block text-blue-600 hover:underline">Go to forgot password</Link></div></div>
  ...
  return (
    <div className="flex min-h-screen min-w-0 items-center justify-center overflow-x-hidden bg-gray-50/90 px-4 py-5 sm:p-4">
      <div className="w-full max-w-[440px] min-w-0 rounded-2xl bg-white p-5 shadow-xl sm:rounded-3xl sm:p-6 md:p-8">
        ...
        <div className="mb-6 sm:mb-8"><h1 className="text-left text-xl font-semibold text-gray-900 sm:text-2xl">Hotel Seller reset password</h1><p className="mt-1 text-left text-sm text-gray-500">Enter OTP sent to {identifier} and set your new password.</p></div>
        <form onSubmit={handleSubmit}>
          ...
          <p className="mt-6 text-center text-sm text-gray-600"><Link href="/hotel-seller/login" className="font-medium text-blue-600 hover:underline">Back to login</Link></p>
        </form>
      </div>
    </div>
  )
...
export default function HotelSellerResetPasswordPage() {
  return <Suspense fallback={<div className="flex min-h-screen items-center justify-center bg-gray-50/90 p-4">Loading...</div>}><HotelSellerResetPasswordForm /></Suspense>
}
