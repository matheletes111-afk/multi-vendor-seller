...
import { LogOut, User, LayoutDashboard, Building2, ShoppingCart, CreditCard, Settings, Menu, Megaphone, Star, Wallet, HelpCircle, FileText } from "lucide-react"
...
const navContent = (
  <>
    ...
    <NavItem href="/hotel-seller/support" label="Support Desk" icon={<HelpCircle className="h-4 w-4" />} />
    <NavItem href="/terms" label="All Terms" icon={<FileText className="h-4 w-4" />} />
  </>
)

export function HotelSellerLayoutClient({
  children,
  user,
}: {
  children: ReactNode
  ...
  return (
    <div className="flex min-h-screen">
      <Sidebar className="hidden md:block">
        <div className="flex items-center justify-center border-b px-6 py-4">
          <a href="/" className="flex w-full flex-col items-center gap-1.5 text-center">
            ...
            <span className="text-xs font-medium text-muted-foreground tracking-wide">Hotel Seller</span>
          </a>
        </div>
      ...
      <div className="flex flex-1 flex-col md:pl-64">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent className="w-56" align="end" forceMount>
                  ...
                  <DropdownMenuItem asChild>
                    <Link href="/hotel-seller/settings"><User className="mr-2 h-4 w-4" /><span>Profile</span></Link>
                  </DropdownMenuItem>
                  <DropdownMenuSeparator />
        ...
        <DashboardFooter panelName="Hotel Host Panel" />
      </div>
    </div>
  )
}
